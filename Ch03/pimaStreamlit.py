...
if upload_file is not None:
    ...
    # st.tex(info_ouput)
    st.write("Processed Data (0 values replaces with mean): ")
    ...
    if st.checkbox("Show Features Importance"):
        ...
        if selected_features:
            ...
            thresold = st.silider("Adjust Threshold", 0.0, 1.0, 0.5, 0.01)

            y_pred_threshold = (y_scores > thresold).astype(int)
            accuracy_threshold = accuracy_score(y_test, y_pred_threshold)
            precision__threshold = precision_score(y_test, y_pred_threshold)
            recall_threshold = recall_score(y_test, y_pred_threshold)

            st.write(f"Performance with threshold {thresold:.2f}")
            st.write(f"- Accuracy: {accuracy_threshold:.2f}")
            st.write(f"- Precision: {precision__threshold:.2f}")
...
if st.checkbox("Show AUC-ROC Curve"):
    ...
    if st.button("Predict"):
        prediction = (model.predic_proba([new_data])[:, 1] > threshold).astype(int)
        result = "Diabetic" if precision[0] == 1 else "Non-Diabetic"
